...
public class Pong implements GameInterface {
    ...
    // player b use w, s and player a uses up and down
    // difficulty can be changed with e, m, h
    // quit = q
    @Override
    ...
    // initialize all fields and the ball
    @Override
    ...
    // checks if and how the ball is moved
    // decides if scored and sounds
    // the x =  5 and width - 5 on both sides are the scoring distance
    ...
    // checks if paddle has been hit
    public boolean hitPlayer(){
        return (ball.nextY() > pongBPos && ball.nextY() < pongBPos + pongLength) || (ball.nextY() > pongAPos && ball.nextY() < pongLength + pongAPos);
    }

    // gamecode = 0 -> normal movement
    // gamecode = 1 -> prints field after scoring
    // gamecode = 2 -> shows score on screen
    @Override
    ...
    // small subclass for ball
    // some physics and randomness implemented
    private class Ball{
        double px;
        double py;
        double vx;
